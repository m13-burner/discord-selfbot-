...
class AIChat(commands.Cog):
    ...
    def _should_reply(self, message: discord.Message) -> bool:
        ...
        if self.mode == "all" and (is_dm or mentioned):
            return True
        return False
    ...
    async def ai_cmd(self, ctx, action: str = "on", *, extra: str = ""):
        ...
        if action in ("on", "enable"):
            ...
            if extra.strip() in valid:
                ...
                msg = await ctx.send(f"Valid modes: `dm` | `mentions` | `channel` | `all`")
            ...
            save_config(cfg)
            msg = await ctx.send(f"🤖 {ctx.channel.mention} {label} AI channels")
